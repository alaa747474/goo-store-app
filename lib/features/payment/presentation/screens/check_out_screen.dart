...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => PaymentCubit(getIt.get<PaymentRepository>())..getPaymentAuthToken(),
      child: Scaffold(
          appBar: CustomAppBar(
            text: 'Checkout',
              ...
              BlocConsumer<PaymentCubit, PaymentState>(
                listener: (context, state) {
                  if (state is PaymentAuthTokenLoaded) {
                    context.read<PaymentCubit>().authToken=state.paymentAuthToken.token;
                    Order order = Order(
                  ...
                  if (state is PaymentFinalTokenLoaded) {
                    Navigator.pushNamed(context, AppRoutes.cardScreen,arguments: state.paymentFinalTokenLoaded);
                  }
                },
                builder: (context, state) {
                  return CustomButton(
                      onPressed: () {
                        print('[SCREEN ======>TOKEN =${context.read<PaymentCubit>().authToken}]');
                        if (state is PaymentOrderRegistrationDone) {
                          ...
                          OrderRegistrationDetails orderRegistrationDetails =
                              OrderRegistrationDetails(
                                integrationId: AppEndPoints.paymentIntegrationId,
                                currency: 'EGP',
